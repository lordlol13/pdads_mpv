...
async def _compute_for_ai(engine, ai_id: int, do_apply: bool) -> None:
    async with engine.connect() as conn:
        ...
        # build a concise topic from title/category and call media selector
        topic_parts = [str(row.get("final_title") or "").strip(), str(row.get("category") or "").strip()]
        topic = " ".join([p for p in topic_parts if p])
        candidates = await fetch_media_urls(topic=topic)
