...
async def try_domain(name: str, domain: str, variants: list[str]):
    ...
    for v in variants:
        ...
        if isinstance(text, str) and text.startswith("__ERROR__:"):
            ...
            continue
        if not text or ('<item' not in text.lower() and '<entry' not in text.lower()):
...
async def main():
    sites = [
        ("Kun.uz", "kun.uz", ["news/rss", "rss", "feed", "rss.xml"]),
        ("Uz24", "uz24.uz", ["rss", "feed", "ru/rss", "feeds"]),
        ("Uznews", "uznews.uz", ["rss", "feed", "rss.xml"]),
        ("Podrobno", "podrobno.uz", ["rss", "feed", "rss.xml"]),
    ]
    for name, domain, variants in sites:
        await try_domain(name, domain, variants)
