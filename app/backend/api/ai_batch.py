...
"""
    ...
    return {"data": ordered}
For each news:
- rewrite summary (max 3 sentences)
- assign category (politics, economy, sports, tech, other)
...
"""
    ...
    # 4) call OpenAI in a threadpool (client may be sync)
    client = _openai_client()
    try:
        resp = await run_in_threadpool(
            lambda: client.chat.completions.create(
        ...
        logger.exception("OpenAI call failed; using fallback summaries")
        # graceful fallback: simple truncation summaries
        for n in to_process:
            item = {"id": n.id, "summary": (n.content or "")[:200].strip() + "...", "category": "other"}
            key = make_cache_key(n, user_id=user_id, user_profile=user_profile)
            try:
                await redis_client.set(key, json.dumps(item, ensure_ascii=False), ex=CACHE_TTL)
            except Exception:
                logger.exception("Redis SET failed while saving fallback summary")
            cached_results[n.id] = item

        ordered = [cached_results[n.id] for n in news_list if n.id in cached_results]
        return {"data": ordered}

    # 5) parse OpenAI response robustly
    parsed = extract_json(content)
    if not parsed:
        logger.warning("Parsed empty list from OpenAI response; returning fallback summaries for to_process")
        for n in to_process:
            item = {"id": n.id, "summary": (n.content or "")[:200].strip() + "...", "category": "other"}
            key = make_cache_key(n, user_id=user_id, user_profile=user_profile)
            try:
    ...
    # 6) save parsed results to Redis
    for item in parsed:
        try:
        ...
        original = next((n for n in to_process if n.id == news_id), None)
        if original is None:
            continue

        key = make_cache_key(original, user_id=user_id, user_profile=user_profile)
        try:
            await redis_client.set(key, json.dumps(item, ensure_ascii=False), ex=CACHE_TTL)
        except Exception:
            logger.exception("Redis SET failed for key=%s", key)

        cached_results[news_id] = item

    # 7) return results in input order
    ordered = [cached_results.get(n.id) for n in news_list if n.id in cached_results]
    logger.info("[AI] processed=%s cached=%s", len(to_process), len(cached_results))
    return {"data": ordered}
